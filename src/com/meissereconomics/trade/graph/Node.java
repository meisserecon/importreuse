...
public class Node {
	...
	public double getCreatedValue() {
		if (isConsumption()) {
			...
			return getOutputsInclConsumption() - getInputs();
		}
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public double calculateComposition(Node consumption, EFlowBendingMode mode, double extent, double epsilon) {
		double input = getInputs();
		double cons = consumption == this ? input : outputs.getDouble(consumption);
		double output = consumption == this ? 0.0 : getOutputsInclConsumption() - cons;
	...
	public double getOutputsInclConsumption() {
		return outputs.getSum();
	}

	public double getInputs() {
		return inputs.getSum();
	}
	...
	public double getReusedImports() {
		return getExports() * origin.getImportReuse();
	}
	...
	private double reduce(Edges outputs) {
	...
	public DoubleArray[] getRelativeShares(final Node other, final boolean input) {
		...
		Edges myMap = (input ? inputs : outputs);
